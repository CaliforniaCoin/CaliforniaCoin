...
uint256 hashGenesisBlock("0x1017569372a0f2c626d602566608fd7e29693c028cb38b3d3ea9bf1a0b93ee1e");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // CaliforniaCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "CaliforniaCoin Signed Message:\n";
...
{
    // CaliforniaCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // 75,600,000 x 10 blocks = 756,000,000 (4.5%) premine for airdrop to California people/dev
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // CaliforniaCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // CaliforniaCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // CaliforniaCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x7b548031469229deb9f421b05a01e4a9dbcf9b30aecc6f17058929f7f53f6e26");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "US FCC Net Neutrality proposes rules";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1400244166;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2090394176;

        if (fTestNet)
        {
            block.nTime    = 1398515841;
            block.nNonce   = 2086817751;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x04e5d81ebd05787161f1ec56334601bfd6f5f9434add9fd7308bc64bc86a91cb"));
// *****************
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Californiacoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CaliforniaCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CaliforniaCoinMiner : ProcessBlock, block not accepted");
    }
...
void static LitecoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running CaliforniaCoin with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CaliforniaCoinMiner terminated\n");
        throw;
    }
}
